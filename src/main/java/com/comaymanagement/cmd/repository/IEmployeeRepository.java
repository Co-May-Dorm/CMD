...
public interface IEmployeeRepository extends JpaRepository<Employee, String> {
	...
	/*
	 *select * from cmd.employees emp inner join cmd.positions_employees pos_emp on emp.id = pos_emp.employee_id 
		inner join cmd.positions pos on pos.id = pos_emp.position_id 
		inner join cmd.departments dep on emp.department_id = dep.id 
		where  emp.name like "%%"  and emp.email like "%%"
		and emp.date_of_birth like "%%"
